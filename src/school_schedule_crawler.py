...
def generate_schedule_html(schedules, school_name, year, month):
    ...
    if total_events == 0:
        ...
        # 일정이 많을 때 글씨 크기 조정을 위한 클래스 추가
        many_events_class = 'many-events' if total_events > 8 else ''
        ...
        if total_events <= 12:
            ...
            event_list_html = {
                ...
                'many_events_class': many_events_class
            }
        else:
            # 12개 초과: 세 부분으로 나누기
    ...
    css_style = '''
        ...
        @media (max-width: 900px) {
            ...
            .calendar-section h2 { font-size: 2.5rem; margin-top: 6px; }
            .schedule-calendar { 
                display: none;
                font-size: 1.8rem; 
            }
        ...
        @media (max-width: 600px) {
            ...
            .calendar-section h2 { font-size: 2rem; margin-top: 6px; }
            .schedule-calendar { 
                display: none;
                font-size: 1.5rem; 
            }
    ...
    if isinstance(event_list_html, dict):
        many_events_class = event_list_html.get('many_events_class', '')
        if event_list_html.get('has_third_part', False):
            ...
            event_list_html_rendered = f'''
                <div class="event-list-container three-columns">
                    <div class="event-list-part">
                        <table class="event-list-table {many_events_class}">
            ...
            event_list_html_rendered = f'''
                <div class="event-list-container">
                    ...
                    {f'<div class="event-list-part"><table class="event-list-table {many_events_class}">{event_list_html["second_part"]}</table></div>' if event_list_html['has_second_part'] else ''}
                </div>
            '''
    else:
        event_list_html_rendered = f'''
