...
"""
학교 홈페이지 공지사항 크롤러
HTML 페이지를 직접 크롤링하여 공지사항을 수집하는 모듈입니다.
"""
...
def crawl_school_notices(url, site_name=None):
    """
    학교 홈페이지 공지사항을 HTML 페이지에서 직접 크롤링합니다.
    
    Args:
        url (str): 공지사항 목록 페이지 URL
    ...
    logging.info(f"{site_name} 공지사항 HTML 크롤러 시작...")
    ...
    try:
        ...
        if not tbody:
            logging.error("공지사항 테이블을 찾을 수 없습니다.")
            return {
                "notices": [],
                "meta": {
                    "total_count": 0,
                        ...
                        if link.startswith('javascript:'):
                            ...
                            if onclick:
                                ...
                                if match:
                                    ...
                                    # 상세보기 URL 생성
                                    link = f"/yulgok-h/na/ntt/selectNttView.do?mi=3812&bbsId=3050&nttSn={ntt_sn}"
                                else:
                                    link = ""
        ...
        logging.info(f"공지사항 HTML 크롤링 완료: {len(notices)}개")
...
if __name__ == "__main__":
    # 율곡고등학교 공지사항 목록 페이지 URL
    test_url = "https://yulgok-h.goepj.kr/yulgok-h/na/ntt/selectNttList.do?mi=3812&bbsId=3050"
    result = crawl_school_notices(test_url, "율곡고등학교")
    
    # 모든 공지사항 출력
    print("\n공지사항 목록:")
    for i, notice in enumerate(result["notices"], 1):
        print(f"{notice.get('title')}\t{notice.get('date')}") 